...
def load_model():
    ...
    state_dict = torch.load('best_model_DenseNet.pth 2', map_location=torch.device('cpu'))
...
def load_model_ca():
    ...
    state_dict = torch.load('best_model_VisionTransformer_test70.pth', map_location=torch.device('cpu'))
...
def process_and_annotate_image(image):
    ...
    max_box_size = (img_width / 1.2, img_height / 1.2)
    min_box_size = (img_width / 5, img_height / 5)
    ...
    for item in result:
        ...
        # 检查框的大小
        if (width <= max_box_size[0] and height <= max_box_size[1] and 
            width >= min_box_size[0] and height >= min_box_size[1] and
            item['score'] > 0 and 
...
def process_and_annotate_image(image):
    # 预处理图像并进行 OCR
    processed_image = process_image(image)
    result = ocr_model.ocr(np.array(processed_image))
    ...
    for item in result:
        ...
        # 检查框的大小
        if width <= max_box_size[0] and height <= max_box_size[1] and width >= min_box_size[0] and height >= min_box_size[1]:
            recognized_texts.append(text)
            bboxes.append(bbox)
    ...
    pil_img = Image.fromarray(image_rgb)
    draw = ImageDraw.Draw(pil_img)
    font = ImageFont.truetype("/Users/jolene/Downloads/SourceHanSerifTC-SemiBold.otf", 48)  # 确保字体路径正确

    centers = []
    for i, (bbox, text) in enumerate(zip(bboxes, recognized_texts)):
        points = np.array(bbox).astype(int)
        polygon = [(p[0], p[1]) for p in points]
        draw.line(polygon + [polygon[0]], fill=(0, 255, 0), width=5)
        
        # 绘制文字内容和编号
        x, y = points[0]
        draw.text((x, y - 25), f"{i+1} {text}", font=font, fill=(255, 0, 0))
    ...
    avg_dist = total_distance / num_distances if num_distances > 0 else 0
...
#         # 如果是路径，使用 cv2.imread 读取
#         image = cv2.imread(image)
#     else:
#         # 如果已经是图像数组，则假定是BGR格式，直接处理
...
def process_image(image):
    ...
    # 用开操作去除独立噪声
    mask_no_noise = cv2.morphologyEx(binary_result, cv2.MORPH_OPEN, kernel)
    
    # 反转图像颜色以便后续处理
    inverted_result = cv2.bitwise_not(mask_no_noise)
    
    # 转换为 RGB 并返回 PIL Image 格式
    rgb_result = cv2.cvtColor(inverted_result, cv2.COLOR_GRAY2RGB)
    pil_image = Image.fromarray(rgb_result)
